...
void Pair::ev_tally_mlp(int i, int j, int nlocal, int newton_pair,
                        double evdwl, double ecoul,
                        double fx, double fy, double fz,
                        double delx, double dely, double delz)
{
  /*fij = -∂V_i/∂r_{ji}=∂V_i/∂r_{ij}, r_{ji}:=r_i-r_j */
  /*delx = x_j - x_i*/
  /*sigma_j += r_{ji} \otimes ∂V_i/∂r_{ij}*/
  /*cvatom_j = -sigma_j*/
  /*cvatom_j += r_{ij} \otimes ∂V_i/∂r_{ij} = delr \otimes fij*/
  if (vflag_either) {
    if (cvflag_atom) {
      cvatom[j][0] += delx*fx;
      cvatom[j][1] += dely*fy;
